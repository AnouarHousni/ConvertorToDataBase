...
{
    internal class SQLServerDataBaseManager : DatabaseManager
    {
        private readonly SqlConnection _sqlConnection;
        private CultureInfo provider = CultureInfo.InvariantCulture;

        public SQLServerDataBaseManager(SqlConnection sqlConnection)
        {
            _sqlConnection = sqlConnection;
        }

        public override async Task OpenConnection() => await _sqlConnection.OpenAsync();
        ...
        public override async Task CreateDatabaseTable(List<TableColumn> databaseolumns, string tableName, string dataBaseName, bool shouldSkipTableExistenceCheck)
        {
            try
            {
                ...
                SqlCommand checkExistsTableCommand = new SqlCommand(query, _sqlConnection);
                ...
                SqlCommand createSqlCommand = new SqlCommand(commandStringCreate, _sqlConnection);
        ...
        public override async Task ExecuteDatabaseQuery(string query)
        {
            try
            {
                using (SqlCommand dbCommand = new SqlCommand(query, _sqlConnection))
                {
        ...
        public override async Task InsertDataIntoDatabase(DataTable dataTable, List<TableColumn> tableColumns, string tableName)
        {
            try
            {
                ...
                foreach (System.Data.DataRow row in dataTable.Rows)
                {
                    ...
                    DbProviderFactory factory = DbProviderFactories.GetFactory(_sqlConnection);
                    ...
                    for (int i = 0; i < Items.Length; i++)
                    {
                        ...
                        parameter.DbType = DbTypeConverter.ConvertToDbType(DataBaseType.SQLSERVER, tableColumns[i].DataType);
                    ...
                    SqlCommand sqlCommandInsert = new SqlCommand(commandStringInsert, _sqlConnection);
